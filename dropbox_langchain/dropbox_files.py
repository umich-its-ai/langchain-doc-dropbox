...
class DropboxLoader(BaseLoader):
    ...
    def _load_pdf_file(self, file_path, download_path, source) -> List[Document]:
        ...
        try:
            ...
            self.logMessage(
                message=f"{file_path} could not be decoded: {str(err)}",
                level="WARNING",
            )
        except Exception as err:
            self.logMessage(
                message=f"Error indexing ({file_path}): {str(err)}",
                level="WARNING",
            )
        ...
        if file_extension in ALLOWED_EXTENSIONS:
            if file_extension == "paper":
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    try:
                        dbx.files_export_to_file(
                            download_path=download_path,
                            path=file_path,
                            export_format="markdown",
                        )
                        file_documents = file_documents + self._load_md_file(
                            file_path, download_path, source
                        )
                    except dropbox.exceptions.DropboxException as error:
                        self.logMessage(
                            message=f"Error exporting ({file_path}): {str(error)}",
                            level="WARNING",
                        )
            else:
                # Download file
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    try:
                        dbx.files_download_to_file(
                            download_path=download_path, path=file_path
                        )

                        if file_extension == "txt":
                            file_documents = file_documents + self._load_text_file(
                                file_path, download_path, source
                            )

                        if file_extension in ["htm", "html"]:
                            file_documents = file_documents + self._load_html_file(
                                file_path, download_path, source
                            )

                        elif file_extension == "pdf":
                            file_documents = file_documents + self._load_pdf_file(
                                file_path, download_path, source
                            )

                        elif file_extension == "docx":
                            file_documents = file_documents + self._load_docx_file(
                                file_path, download_path, source
                            )

                        elif file_extension in ["xlsx", "xls"]:
                            file_documents = file_documents + self._load_excel_file(
                                file_path, download_path, source
                            )

                        elif file_extension == "pptx":
                            file_documents = file_documents + self._load_pptx_file(
                                file_path, download_path, source
                            )
                            ...
                            file_documents = file_documents + self._load_rtf_file(
                                file_path, download_path, source
                            )

                    except dropbox.exceptions.DropboxException as error:
                        self.logMessage(
                            message=f"Error downloading ({file_path}): {str(error)}",
                            level="WARNING",
                        )
    ...
    def _load_files_from_folder_path(self, dbx, folder_path) -> List[Document]:
        self.logMessage(
            message=f"Load files from folder path ({folder_path})", level="DEBUG"
        )
        ...
        try:
            ...
            file_documents = self._load_files_from_paths(dbx=dbx, file_paths=file_paths)
        except dropbox.exceptions.DropboxException as error:
            self.logMessage(
                message=f"Error listing folder ({folder_path}): {str(error)}",
                level="WARNING",
            )
    ...
    def _load_files_from_paths(self, dbx, file_paths) -> List[Document]:
        self.logMessage(
            message=f"Load files from file paths (count: {len(file_paths)})",
            level="DEBUG",
        )
    ...
    def list_folders(self, dbx, root_namespace_id=None):
        ...
        try:
            ...
            found_all_records = False
            dbx = (
                dbx.with_path_root(dropbox.common.PathRoot.root(root_namespace_id))
                        ...
                        entries_info.append(
                            {
                                ...
                                "parent_shared_folder_id": (
                                    entry.parent_shared_folder_id
                                    if hasattr(entry, "parent_shared_folder_id")
                                    else None
                                ),
                            }
                        )
            ...
            self.logMessage(
                message=f"Error listing folders: {str(error)}", level="WARNING"
            )
        return entries_info
    ...
    def get_folders(self):
        ...
        try:
            ...
            self.logMessage(
                message=f"Error getting folders: {str(error)}", level="WARNING"
            )
            return []
    ...
    def load(self) -> List[Document]:
        ...
        try:
            ...
            self.logMessage(
                message="Invalid Dropbox credentials. Please allow access your Dropbox account.",
                level="ERROR",
            )
            raise
        except dropbox.exceptions.DropboxException as error:
        ...
        try:
            ...
            dbx_team = dropbox.Dropbox(
                oauth2_access_token=self.auth["access_token"], headers=headers
            )
        except dropbox.exceptions.DropboxException as error:
            self.logMessage(
